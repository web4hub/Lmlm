"""Lola Script.god participant — HTTP+JSON transport, executor role only
(no ROUTE/COORDINATE/POLICY — those belong to a coordinator like rodaai-agent)."""
...
PARTICIPANT_ID = "lola"
app = FastAPI(title="Lola Script.god participant")
...
async def receive(envelope: Envelope):
    ...
    if envelope.type == MessageType.CAPABILITIES:
        return {
            ...
            "payload": {
                ...
                "pipeline_stages": ["perceive", "understand", "retrieve", "execute", "verify", "remember"],
    ...
    if envelope.type == MessageType.TASK:
        ...
        return {"type": "RESULT", "payload": {"output": result, "verified": ok, "stages_completed": ["perceive", "understand", "retrieve", "execute", "remember"]}}

    if envelope.type == MessageType.CANCEL:
        # TODO: abort any in-flight task for this correlation_id
